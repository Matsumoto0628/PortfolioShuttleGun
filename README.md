...
好きな3Dアクションゲームのようなスライディングや壁走りなどの
多様なキャラクターコントロールを
自分ならどう実装するかという好奇心がきっかけとなった。

3Dにおけるベクトル演算とアニメーション制御を重視し、制作した。

また、UniRxでMVPパターンのUIを実装したり、FSMでAIの制御を実装したり
銃の実装でポリモーフィズムを考えたり、
拡張しやすく、エラーが見つけやすい設計を意識して制作することを目標にした。
...
UniTaskはUnityのライフサイクルに統合された async/await を提供し、ゼロアロケーションでパフォーマンスへの影響も小さいため選定した。
...
速度の加算には `Vector3.Project` で現在速度を目標方向へ射影することで、方向成分のみを抽出して加速量を計算している。
